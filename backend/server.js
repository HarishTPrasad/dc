...
app.use(
  cors({
    origin: process.env.CLIENT_URL || "http://localhost:2025", // Default to localhost if not set
  })
);
app.use(express.json()); // Parse JSON request bodies

// MongoDB Connection
const MONGO_URI = process.env.MONGO_URI || "mongodb://mongo:27017/mernapp"; // Use 'mongo' as the Docker service name
